...
- 作品名を決める
- 各メンバーでフロアごとに作成する
- clusterにアップロードして動作確認
- メンバーが揃ったら以下を実施
  - 床を作成してエクスポートしてプロジェクトを統合
  - 高さ、階段の位置を合わせる
  - 空を設定する
  - 以上を各自の作業プロジェクトに反映させて、残りを作る
...
- 作品名を決める
- ステージとプレイヤーを統合
- タイトル、ゲーム開始、ミスによるゲームオーバー、ステージクリア、全ステージクリアを作成して、シーンが循環するように設定

### Barrage(仮)
- プレイヤーの実装(ショット、画面外に出ないように、ミス)
- 敵を１種類＋弾を撃たせる
- 敵をランダムで出現させて、サバイバルでハイスコアを競えるようにする
